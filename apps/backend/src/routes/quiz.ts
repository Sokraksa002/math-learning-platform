...
import { startQuiz, submitQuiz, getQuizResult, QuizSessionCompletedError } from "../modules/quiz/quiz.service";
import { z } from "zod";
import {
  startQuizSchema,
  submitQuizSchema,
  ...
  if (!ensureExistsAndOwned(session, userId, reply, "Quiz session")) return;
      ...
      try {
        ...
        if (err instanceof QuizSessionCompletedError) {
          reply.code(409).send({ code: 'QUIZ_COMPLETED', message: 'Quiz session already completed' });
          return;
        }
        throw err;
      }
