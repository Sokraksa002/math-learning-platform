  ...
  return (
    ...
    >
      <Container maxWidth="xl" sx={{ py: 4, flex: 1 }}>
        <Paper sx={{ p: { xs: 2.5, md: 3.5 }, mb: 3, borderRadius: 4, color: 'white', background: 'linear-gradient(135deg, #0f172a 0%, #1d4ed8 45%, #2563eb 100%)', boxShadow: '0 18px 40px rgba(37, 99, 235, 0.22)' }}>
          <Typography variant="overline" sx={{ opacity: 0.85, letterSpacing: 1.5, fontWeight: 700 }}>
            {isKhmer ? 'កាតរំលឹក' : t('pages.Flashcard.flashcards', 'Flashcards')}
          </Typography>
          <Typography variant="h4" fontWeight={900} sx={{ mt: 1, lineHeight: 1.1 }}>
            {isKhmer
              ? 'អនុវត្តជាមួយប្រធានបទសិក្សាពណ៌ខៀវ'
              : t('pages.Flashcard.practice_theme', 'Practice with the blue learning theme')}
          </Typography>
          <Typography sx={{ mt: 1, opacity: 0.9, maxWidth: 880, lineHeight: 1.7 }}>
            {isKhmer
